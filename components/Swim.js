  ...
  const handleSubmit = (e) => {
    ...
    if (selectedTwoHundredTime && selectedFourHundredTime) {
      try {
        ...
        const calculatedCSS = calculateCSS(
          selectedTwoHundredTime,
          selectedFourHundredTime
        );
        const data = createTableData(
          descriptions,
          createPaces(calculatedCSS, scaler)
        );
    ...
    <div className="flex flex-col items-center justify-center">
      ...
      <form onSubmit={handleSubmit} className="w-11/12 md:w-full">
        <div className="grid grid-cols-1">
          <label
            htmlFor="200Time"
            className="capitalize text-lg justify-self-center md:text-2xl"
          >
            200 Time
          </label>
          <select
      ...
      {showAlert && (
        <div className="flex justify-center">
          <div className="w-1/2 flex justify-center bg-red-100 border border-red-400 text-red-700 px-4 py-2 mt-4 rounded">
            <span>
              Please input both 200 Time and 400 Time before submitting the form
            </span>
            <button
              onClick={closeAlert}
              className="float-right text-red-700 font-semibold ml-2"
            >
              X
            </button>
          </div>
        ...
        <div className="flex flex-col">
          <div className="mt-9 mx-auto">
            <p className="text-lg md:text-2xl">
              Your CSS swim pace is{" "}
              <span className="font-bold">{result} per 100</span>
            </p>
          </div>
      ...
      <div className="mx-5 mt-10 md:mt-36 md:mx-32">
        ...
        <p className="text-center text-lg md:text-2xl md:mt-10 mt-2">
          ...
          test. The CSS test involves swimming a series of timed distances, a
          400 and a 200, with a rest interval in between. The objective is to
          swim these distances at a consistent pace, pushing yourself to
          maintain the fastest sustainable speed. Record the time it takes to
          complete each distance, and use these times in the calculator above.
          Your CSS pace represents the pace you should be able to sustain for
          about a 1500 all out.
        </p>
